...
<html lang="pt-br" data-bs-theme="dark">
  <head>
    ...
    <title>Servidores - Help Desk</title>
  ...
  <body class="min-vh-100 d-flex flex-column">
    <nav class="navbar navbar-expand-lg glass">
      <div class="container-fluid">
        <a class="navbar-brand" href="/portal/helpdesk/index.php"><?= render_brand_html(__DIR__ . '/../../assets/logotipo.png'); ?></a> | HELP DESK | Servidores
    ...
    <main class="container py-4">
      ...
      <div class="text-secondary small mt-2">Os servidores são geridos em Configurações &gt; Servidores. Aqui você pode iniciar análises.</div>
    </main>
    ...
    <script>
      ...
      async function load(){
        ...
        try{
          ...
          const it=d.items||[];
          if(it.length===0){tb.innerHTML='<tr><td colspan="7" class="text-secondary">Sem servidores.</td></tr>';return;}
          tb.innerHTML=it.map(s=>{
            ...
            return `<tr data-id='${s.id}'><td>${s.id}</td><td>${h(s.name||'')}</td><td>${h(s.host||'')}</td><td>${h(s.ip||'')}</td><td class='small text-secondary'>${h(base)}</td><td class='small' id='st-${s.id}'><span class='badge bg-secondary'>...</span></td><td>
              ...
              <a class='btn btn-sm btn-outline-light' href='server_tools.php?id=${s.id}'>Ferramentas</a>
            </td></tr>`;
          }).join('');
          // fetch status
